...
- lemon07r/VellumK2-Fantasy-DPO-Tiny-01 - **A tiny 128 row dataset that can be useful for testing or validation**
- lemon07r/VellumK2-Fantasy-DPO-Small-01 (in progress) - **A small 1k row dataset that can be useful light training or in combination with other datasets**
...
### Custom Story Generation Templates

Customize how stories are generated by editing templates in `config.toml`:
...
- `{{.MainTopic}}` - The main topic from config
- `{{.SubTopic}}` - The generated subtopic
- `{{.Prompt}}` - The generated writing prompt
